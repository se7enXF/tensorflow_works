...
filewriter_path = "D:/tf_work/log/class{}_{}_lr_{}".format(num_classes, datetime.now().strftime("%Y-%m-%d_%H-%M"), learning_rate)
...
# Initialize model
model = AlexNet(x, keep_prob, train_layers, n_class=num_classes)
...
train_generator = ImageDataGenerator(train_file, n_class=num_classes, shuffle=True)
val_generator = ImageDataGenerator(val_file, n_class=num_classes, shuffle=False)
